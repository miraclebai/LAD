...
def _build_radial_rect_mask(
    H: int, W: int,
    x1: int, y1: int, x2: int, y2: int,
    mode: str = "gaussian",   # "gaussian" or "power"
    sigma: float = 0.55,      # only for gaussian (0.3~0.8 常用)
    gamma: float = 2.0,       # only for power (1~6 常用)
    device="cpu",
) -> torch.Tensor:
    """
    Rectangular ROI, but haze intensity is radial from ROI center (strong at center, weak to boundary).
    ...
    # ROI grid
    yy = torch.arange(y1, y2, device=device).float()
    xx = torch.arange(x1, x2, device=device).float()
    ...
    # normalized radial distance in ROI coordinates
    nx = (X - cx) / half_w
    ny = (Y - cy) / half_h
    r = torch.sqrt(nx * nx + ny * ny)  # center=0, boundary ~1, corners ~sqrt(2)

    if mode == "gaussian":
        # sigma controls spread; larger sigma => flatter / wider haze
        sig = max(1e-6, float(sigma))
        roi_mask = torch.exp(-(r * r) / (2.0 * sig * sig))
    elif mode == "power":
        # gamma controls sharpness; larger gamma => more concentrated center
        g = max(1e-6, float(gamma))
        roi_mask = torch.clamp(1.0 - r, min=0.0) ** g
    else:
        raise ValueError(f"Unknown mode: {mode}")

    # Normalize within ROI to [0,1]
    roi_mask = roi_mask / (roi_mask.max() + 1e-8)
    m[:, :, y1:y2, x1:x2] = roi_mask.unsqueeze(0).unsqueeze(0)
    return m.clamp(0, 1)
...
def main():
    ...
    # x=276, y=198, w=104, h=39
    # x=352, y=76, w=66, h=66
    ap.add_argument("--outdir", type=str, default="/Users/baijingyuan/Desktop/result/vis_roi_diffusion_min_1_re2")
    ...
    ap.add_argument("--delta_rgb", type=float, default=1,
    ...
    ap.add_argument("--roi_mask", type=str, default="radial_gaussian",
                choices=["soft_rect", "radial_gaussian", "radial_power"])

    ap.add_argument("--radial_sigma", type=float, default=0.35,
                    help="for radial_gaussian: spread of haze (0.3~0.8)")

    ap.add_argument("--radial_gamma", type=float, default=7,
                    help="for radial_power: sharpness (1~6)")
    ...
    # 4) soft mask
    if args.roi_mask == "soft_rect":
        mask = _build_soft_roi_mask(H, W, x1, y1, x2, y2,
                                    feather=args.feather, device=dev)
    elif args.roi_mask == "radial_gaussian":
        mask = _build_radial_rect_mask(H, W, x1, y1, x2, y2,
                                    mode="gaussian",
                                    sigma=args.radial_sigma,
                                    device=dev)
    elif args.roi_mask == "radial_power":
        mask = _build_radial_rect_mask(H, W, x1, y1, x2, y2,
                                    mode="power",
                                    gamma=args.radial_gamma,
                                    device=dev)
    else:
        raise ValueError(args.roi_mask)
