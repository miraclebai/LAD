...
def main():
	...
	parser.add_argument(
		...
		default="/disk8t/baijy/dataset/Hazyval/hazy",
	)
	parser.add_argument(
		"--outdir",
		type=str,
		...
		default="results/output_infer_1",
	)
	parser.add_argument(
		"--ddpm_steps",
		type=int,
		...
		with precision_scope("cuda"):
			with model.ema_scope():
				...
				for n in trange(niters, desc="Sampling"):
					...
					struct_cond = m_l
					samples, _ = model.sample(cond=semantic_c, struct_cond=struct_cond, batch_size=init_image.size(0), timesteps=opt.ddpm_steps, time_replace=opt.ddpm_steps, x_T=x_T, return_intermediates=True)
					posterior, enc_fea, m_l = vq_model.encode(init_image)	
					# x_samples = vq_model.decode(samples, enc_fea)
