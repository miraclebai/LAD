...
from data.data import ResizePairedDataset
...
def main():
    ...
    # data
    dataset = ResizePairedDataset(config.test_data.params.test.params)
            ...
            with model.ema_scope():
                ...
                for batch_idx, samples in enumerate(test_dataloader):
                    if os.path.exists(os.path.join(latent_path, f"{base_i:08}.npy")):
                        ...
                        # 不用文本条件，直接设为 None，当作无条件模型
                        semantic_c = model.cond_stage_model(text_init)


                        noise = torch.randn_like(init_latent)
                        t = repeat(torch.tensor([999]), '1 -> b', b=size)
                        t = t.to(device).long()
                        x_T = model_ori.q_sample(x_start=init_latent, t=t, noise=noise)
                        # x_T = None

                        samples, _ = model.sample(cond=semantic_c, struct_cond=init_latent, batch_size=size, timesteps=opt.ddpm_steps, time_replace=opt.ddpm_steps, x_T=x_T, return_intermediates=True, adain_fea=None)
