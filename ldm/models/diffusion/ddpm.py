...
class DDPM(pl.LightningModule):
    ...
    def __init__(self,
        ...
        if self.learn_logvar:
            ...
            self.register_buffer("logvar", logvar)   # ✅ 让它随模型迁移到 GPU
...
class LatentDiffusion(DDPM):
    ...
    def to_rgb(self, x):
        ...
        return x
class LatentDiffusionDehazing(DDPM):
    """main class"""
    def __init__(self,
                 first_stage_config,
                 cond_stage_config,
        ...
        if self.structcond_stage_model is not None:
    ...
    def get_input(self, batch, k=None, return_first_stage_outputs=False, force_c_encode=False,
        ...
        """Flare Corruption pipeline, copied from Flare7K++:
        https://github.com/ykdai/Flare7K
        """

        # gt = groud truth; lq = hazy
        im_gt = batch['gt'].cuda()
        lq = batch.get('hazy', batch.get('lq')).cuda()
        ...
        self.gt = torch.clamp(self.gt, -1.0, 1.0)
    ...
    def shared_step(self, batch, **kwargs):
        # gt is z_gt, c is text_cond, x is z_hazy, im_gt is im_gt, lq is im_hazy
        gt, c, x, im_gt, lq = self.get_input(batch, self.first_stage_key)
        loss, loss_dict = self(x, c, gt)
        return loss, loss_dict
    ...
    def apply_model(self, x_noisy, t, cond, struct_cond, return_ids=False):
        if cond is None or self.model.conditioning_key is None:
            x_recon = self.model(x_noisy, t, struct_cond=struct_cond)
            if isinstance(x_recon, tuple) and not return_ids:
                return x_recon[0]
            else:
                return x_recon
    ...
    @torch.no_grad()
    def log_images(self, batch, N=12, n_row=4, sample=True, ddim_steps=200, ddim_eta=1., return_keys=None,
                   quantize_denoised=True, inpaint=False, plot_denoise_rows=False, plot_progressive_rows=False,
                   plot_diffusion_rows=False, **kwargs):

        use_ddim = ddim_steps is not None
        log = dict()
        z_gt, c_lq, z_lq, im_gt, lq = self.get_input(batch, self.first_stage_key,
                                           return_first_stage_outputs=True,
                                           force_c_encode=True,
                                           return_original_cond=True,
                                           bs=N, val=True)
        N = min(lq.shape[0], N)
        n_row = min(lq.shape[0], n_row)
        if self.test_gt:
            log["gt"] = im_gt
        else:
            log["inputs"] = lq
        ...
        if sample:
            ...
            if inpaint:
                ...
                # make a simple center square
                b, h, w = z_gt.shape[0], z_gt.shape[2], z_gt.shape[3]
                mask = torch.ones(N, h, w).to(self.device)
                # zeros will be filled in
                mask[:, h // 4:3 * h // 4, w // 4:3 * w // 4] = 0.
                mask = mask[:, None, ...]
...
class DiffusionWrapper(pl.LightningModule):
    ...
    def forward(
        ...
        # ① 无条件：只用结构条件（去雾时就只用 hazy 的 latent）
        if self.conditioning_key is None:
            out = self.diffusion_model(x, t, struct_cond=struct_cond)

        # ② concat 条件：把条件直接在通道维拼到 x 上，同时传 struct_cond
        elif self.conditioning_key == 'concat':
            assert c_concat is not None
            xc = torch.cat([x] + c_concat, dim=1)
            out = self.diffusion_model(xc, t, struct_cond=struct_cond)

        # ③ cross-attn 条件：文本 / 语义 走 crossattn，同时传 struct_cond
        elif self.conditioning_key == 'crossattn':
            # 原来：cc = torch.cat(c_crossattn, 1); context=cc
            # 兼容你 attention.py 里用 context[i] 的写法：传 list，让 context[0] 仍是 [B,77,1024]
            if isinstance(c_crossattn, (list, tuple)):
                if len(c_crossattn) == 1:
                    context = list(c_crossattn)            # [c]  (c 是 [B,77,1024])
                else:
                    context = [torch.cat(list(c_crossattn), dim=1)]
            else:
                # 极端情况：有人直接把 tensor 传进来
                context = [c_crossattn]

            out = self.diffusion_model(x, t, context=context, struct_cond=struct_cond)

        elif self.conditioning_key == 'hybrid':
            # 原来：cc = torch.cat(c_crossattn, 1); xc = torch.cat(c_concat, 1); context=cc
            x = torch.cat([x] + list(c_concat), dim=1)

            if isinstance(c_crossattn, (list, tuple)):
                if len(c_crossattn) == 1:
                    context = list(c_crossattn)
                else:
                    context = [torch.cat(list(c_crossattn), dim=1)]
            else:
                context = [c_crossattn]
...
def space_timesteps(num_timesteps, section_counts):
    ...
    for i, section_count in enumerate(section_counts):
        ...
        start_idx += size
    return set(all_steps)
class HazeLatentDiffusion(DDPM):
    """LatentDiffusionDehazing(DDPM) 的最小修改版：
       - first_stage_model: HazeAutoencoderKLResi (encode/decode 接口不同)
       - struct_cond 输入: mass map M (而不是 z_lq/z_gt)
       - 其余逻辑尽量与原版一致，便于逐函数对照
    """
    ...
    def __init__(self,
        ...
        # --- cache (用于 log_images / decode) ---
        self._last_enc_fea_lq = None
        self._last_enc_fea_gt = None
        self._last_m_lq = None
        self._last_m_gt = None
        ...
        self.register_schedule(
            given_betas=np.array(new_betas),
            timesteps=len(new_betas),
    ...
    # -------------------------------------------------------------------------
    # ====== 新增：适配 HazeAutoencoderKLResi.encode/decode 的工具函数 ======
    # -------------------------------------------------------------------------
    def _unpack_first_stage_encode(self, enc_out):
        """
        兼容：
          - old: posterior (DiagonalGaussianDistribution / Tensor)
          - new: (posterior, enc_fea, m_l) 或 (posterior, enc_fea, m_l, ...)
        返回: posterior, enc_fea(or None), m_l(or None)
        """
        if isinstance(enc_out, (tuple, list)):
            posterior = enc_out[0]
            enc_fea = enc_out[1] if len(enc_out) > 1 else None
            m_l = enc_out[2] if len(enc_out) > 2 else None
            return posterior, enc_fea, m_l
        else:
            return enc_out, None, None

    def _prep_mass_for_structcond(self, m):
        """
        structcond_stage_model 期望输入通常在 [-1,1]。
        假设 m 原本在 [0,1]，则映射到 [-1,1]。
        """
        if m is None:
            return None
        if m.dim() == 3:
    ...
    def _sanitize_enc_fea(self, enc_fea, B):
        """
        解决你遇到的 3D vs 4D：
        - 把 [C,H,W] -> [1,C,H,W]
        - 确保 batch 维存在且能 broadcast 到 B
        """
        if enc_fea is None:
            return None
        if isinstance(enc_fea, (tuple, list)):
            out = []
            for f in enc_fea:
                if f is None:
                    out.append(None)
            ...
            return out
        # 单 tensor
        if torch.is_tensor(enc_fea) and enc_fea.dim() == 3:
            enc_fea = enc_fea.unsqueeze(0)
        return enc_fea

    # -------------------------------------------------------------------------
    # 原版函数：cond schedule / register_schedule / instantiate_xxx
    # -------------------------------------------------------------------------
    def make_cond_schedule(self, ):
        self.cond_ids = torch.full(size=(self.num_timesteps,), fill_value=self.num_timesteps - 1, dtype=torch.long)
        ids = torch.round(torch.linspace(0, self.num_timesteps - 1, self.num_timesteps_cond)).long()
    ...
    def on_train_batch_start(self, batch, batch_idx, dataloader_idx):
        ...
        if self.scale_by_std and self.current_epoch == 0 and self.global_step == 0 and batch_idx == 0 and not self.restarted_from_ckpt:
            ...
            x = super().get_input(batch, self.first_stage_key)
            x = x.to(self.device)
            enc_out = self.encode_first_stage(x)
            posterior, _, _ = self._unpack_first_stage_encode(enc_out)
    ...
    def get_first_stage_encoding(self, encoder_posterior):
        if isinstance(encoder_posterior, (tuple, list)):
            encoder_posterior = encoder_posterior[0]
        if isinstance(encoder_posterior, DiagonalGaussianDistribution):
            z = encoder_posterior.sample()
        elif isinstance(encoder_posterior, torch.Tensor):
            z = encoder_posterior
    ...
    # -------------------------------------------------------------------------
    # ✅ 修改点：get_input —— 同时拿到 enc_fea 与 m_l，并缓存起来
    # -------------------------------------------------------------------------
    @torch.no_grad()
    def get_input(self, batch, k=None, return_first_stage_outputs=False, force_c_encode=False,
                  cond_key=None, return_original_cond=False, bs=None, val=False, text_cond=[''], return_gt=False, resize_lq=True,
                  return_struct_cond=True, return_enc_fea=True):
        ...
        # --- encode gt ---
        self.gt = self.gt.to(memory_format=torch.contiguous_format).float()
        enc_out_gt = self.encode_first_stage(self.gt)
        posterior_gt, enc_fea_gt, m_gt = self._unpack_first_stage_encode(enc_out_gt)
        z_gt = self.get_first_stage_encoding(posterior_gt).detach()

        # --- encode lq(hazy) ---
        self.lq = self.lq.to(memory_format=torch.contiguous_format).float()
        enc_out_lq = self.encode_first_stage(self.lq)
        posterior_lq, enc_fea_lq, m_lq = self._unpack_first_stage_encode(enc_out_lq)
        z_lq = self.get_first_stage_encoding(posterior_lq).detach()

        # cache for log_images / debug
        self._last_enc_fea_gt = enc_fea_gt
        self._last_enc_fea_lq = enc_fea_lq
        self._last_m_gt = m_gt
        self._last_m_lq = m_lq

        # text cond list length match
        while len(text_cond) < z_gt.size(0):
            text_cond.append(text_cond[-1])
        if len(text_cond) > z_gt.size(0):
            text_cond = text_cond[:z_gt.size(0)]
        ...
        # 保持原版返回结构（便于 log_images/外部复用）
        # 原版 out = [z_gt, text_cond, z_lq, self.gt, self.lq]
        out = [z_gt, text_cond, z_lq, self.gt, self.lq]

        # 若训练/调用方需要额外内容，可在末尾附加（不会破坏原5元组使用）
        if return_struct_cond:
            out.append(m_gt)
            out.append(m_lq)
        if return_enc_fea:
            out.append(enc_fea_gt)
            out.append(enc_fea_lq)

        return out

    # -------------------------------------------------------------------------
    # ✅ 修改点：decode_first_stage —— 支持 enc_fea，并修复 3D/4D 维度问题
    # -------------------------------------------------------------------------
    @torch.no_grad()
    def decode_first_stage(self, z, predict_cids=False, force_not_quantize=False, enc_fea=None):
        if predict_cids:
            if z.dim() == 4:
                z = torch.argmax(z.exp(), dim=1).long()
            z = self.first_stage_model.quantize.get_codebook_entry(z, shape=None)
        ...
        if enc_fea is not None:
            enc_fea = self._sanitize_enc_fea(enc_fea, B=z.shape[0])

        if hasattr(self, "split_input_params"):
            if self.split_input_params["patch_distributed_vq"]:
                # ⚠️ 若你启用了 patch_distributed_vq，HazeAutoencoderKLResi 的 enc_fea 对齐非常复杂
                #    这里先按原版逻辑走，但不支持 per-patch enc_fea（会忽略 enc_fea）
                ...
                for i in range(z.shape[-1]):
                    try:
                        output_list.append(self.first_stage_model.decode(z[:, :, :, :, i], None))
                    except TypeError:
                        output_list.append(self.first_stage_model.decode(z[:, :, :, :, i]))
    ...
    def differentiable_decode_first_stage(self, z, predict_cids=False, force_not_quantize=False, enc_fea=None):
        if predict_cids:
            if z.dim() == 4:
                z = torch.argmax(z.exp(), dim=1).long()
            z = self.first_stage_model.quantize.get_codebook_entry(z, shape=None)
    ...
    # -------------------------------------------------------------------------
    # ✅ 修改点：encode_first_stage —— 允许返回 (posterior, enc_fea, m_l)
    # -------------------------------------------------------------------------
    @torch.no_grad()
    def encode_first_stage(self, x):
        if hasattr(self, "split_input_params"):
            if self.split_input_params["patch_distributed_vq"]:
                # 同理：若 patch 模式，m_l/enc_fea 的 stitching 很复杂，这里按原版返回 stitched posterior
                ks = self.split_input_params["ks"]
                stride = self.split_input_params["stride"]
                df = self.split_input_params["vqf"]
                self.split_input_params['original_image_size'] = x.shape[-2:]
                ...
                output_list = []
                for i in range(z.shape[-1]):
                    out_i = self.first_stage_model.encode(z[:, :, :, :, i])
                    post_i, _, _ = self._unpack_first_stage_encode(out_i)
                    output_list.append(post_i)
    ...
    # -------------------------------------------------------------------------
    # ✅ 修改点：shared_step / forward —— struct_cond 用 M，而不是 z_lq/z_gt
    # -------------------------------------------------------------------------
    def shared_step(self, batch, **kwargs):
        # get_input 现在会在末尾附加 m_gt, m_lq, enc_fea_gt, enc_fea_lq
        out = self.get_input(batch, self.first_stage_key, return_struct_cond=True, return_enc_fea=True)
        # 兼容原 5 项
        z_gt, c, z_lq, im_gt, lq = out[:5]
        m_gt, m_lq = out[5], out[6]
        # 训练时 struct_cond_source 选用 hazy 的 M（或 test_gt 用 gt 的 M）
        m_source = m_gt if self.test_gt else m_lq

        loss, loss_dict = self(z_lq, c, z_gt, m_source)
        return loss, loss_dict

    def forward(self, x, c, gt, m_source=None, *args, **kwargs):
        # x: z_lq, gt: z_gt, m_source: mass map (latent-res) from encoder
        index = np.random.randint(0, self.num_timesteps, size=x.size(0))
        t = torch.from_numpy(index).to(self.device).long()

        t_ori = torch.tensor([self.ori_timesteps[index_i] for index_i in index]).long().to(x.device)
        ...
        # struct_cond: 由 M 进入 structcond_stage_model
        if m_source is None:
            # 兼容旧调用：退回用 x 作为 struct 输入（不推荐）
            m_source = x

        m_source = self._prep_mass_for_structcond(m_source)
        struc_c = self.structcond_stage_model(m_source, t_ori)

        return self.p_losses(gt, c, struc_c, t, t_ori, gt, *args, **kwargs)

    # -------------------------------------------------------------------------
    # apply_model / p_losses / p_mean_variance / p_sample... 绝大多数不需要改
    # （struct_cond 的来源已经在 forward / p_sample_loop 中解决）
    # -------------------------------------------------------------------------
    def apply_model(self, x_noisy, t, cond, struct_cond, return_ids=False):
        if cond is None or self.model.conditioning_key is None:
            x_recon = self.model(x_noisy, t, struct_cond=struct_cond)
        ...
        if hasattr(self, "split_input_params"):
            ...
            if self.cond_stage_key in ["image", "LR_image", "segmentation", 'bbox_img'] and self.model.conditioning_key:
                ...
                raise NotImplementedError("coordinates_bbox branch unchanged but omitted here for brevity in explanation")
            else:
    ...
    # ====== 下面大量函数与你原版一致（p_losses / p_mean_variance / p_sample / sample / log_images / configure_optimizers / to_rgb）=====
    # 为避免你对照困难，我保留你原版实现，唯一需要动的地方是：
    #   - p_sample_loop / sample / log_images 里 struct_cond 应当传入 M (mass map)
    #   - log_images 里 recon/decode 可以传 enc_fea（尤其是 recon_lq/recon_gt）
    #
    # 你下面原版函数可直接粘贴过来，我只在 p_sample_loop/sample/log_images 三处做最小改动。
    # -------------------------------------------------------------------------
    ...
    def p_sample_loop(self, cond, struct_cond, shape, return_intermediates=False,
        ...
        # ✅ struct_cond 现在是 mass map M（latent-res），需要在每步输入 struct encoder
        struct_cond = self._prep_mass_for_structcond(struct_cond)
        ...
        for i in iterator:
            ...
            if t_replace is not None:
                if start_T is not None and self.ori_timesteps[i] > start_T:
                    continue
                struct_cond_input = self.structcond_stage_model(struct_cond, t_replace)
            else:
    ...
    def sample(self, cond, struct_cond, batch_size=16, return_intermediates=False, x_T=None,
        ...
        # ✅ struct_cond 现在就是 M（mass map），而不是 z_lq
        return self.p_sample_loop(cond,
                                  struct_cond,
                                  shape,
                                  return_intermediates=return_intermediates, x_T=x_T,
    ...
    def log_images(self, batch, N=12, n_row=4, sample=True, ddim_steps=200, ddim_eta=1., return_keys=None,
        ...
        z_gt, c_lq, z_lq, im_gt, lq = out[:5]
        m_gt, m_lq = out[5], out[6]
        enc_fea_gt, enc_fea_lq = out[7], out[8]
        ...
        if self.test_gt:
            ...
            log["inputs"] = lq
            log["reconstruction"] = im_gt
            # ✅ recon_lq 用 enc_fea_lq 解码（避免你遇到的维度错 / 或者重建质量差）
            log["recon_lq"] = self.decode_first_stage(z_lq, enc_fea=enc_fea_lq)
            recon_lq = log["recon_lq"]

        c = self.cond_stage_model(c_lq)

        # ✅ struct_cond = mass map M
        struct_cond = m_gt if self.test_gt else m_lq
        ...
        if plot_diffusion_rows:
            ...
            for t in range(self.num_timesteps):
                if t % self.log_every_t == 0 or t == self.num_timesteps - 1:
                    ...
                    diffusion_row.append(self.decode_first_stage(z_noisy, enc_fea=None))
        ...
        if sample:
            ...
            ddim_sampler = DDIMSampler(self)
            with self.ema_scope("Plotting"):
                cur_time_step = self.time_replace if (self.time_replace is not None) else 1000
                samples, z_denoise_row = self.sample(
                    cond=c,
                    struct_cond=struct_cond,  # ✅ M
                    batch_size=N,
                    timesteps=cur_time_step,
                    return_intermediates=True,
                    time_replace=self.time_replace
                )

            # ✅ 1) merge 前：纯 diffusion 输出（decode 后）
            x_samples_raw = self.decode_first_stage(samples, enc_fea=None)
            log["samples_raw"] = x_samples_raw

            # ✅ 2) merge 后：你现在原来保存的 samples
            if not self.test_gt:
                x_samples_merged = merge(recon_lq, x_samples_raw, lq)
            else:
                x_samples_merged = x_samples_raw
            log["samples"] = x_samples_merged
            # 可选：也单独存个名字更明确的
            log["samples_merged"] = x_samples_merged
